...
class TestPosts:
    ...
    def test_expected_200_get_one_posts(self, api: ApiClient, get_one_schema) -> None:
        ...
        assert res.status_code == 200, "status code != 200"
        assert res.json()["id"] == data, "Not match id"
        assert Schema.validate(res.json(), get_one_schema) is True,"Not match schema"

    @pytest.mark.xfail
    def test_expect_not_found_posts(self, api: ApiClient):
        data = randint(500, 1000)
        res = api.get_posts(path_param=f"/{data}")
        assert res.status_code == 404, "status code != 404"
        assert res.json() == {}, "json not empty"
    ...
    def test_expected_201_create_posts(self, api: ApiClient, post_schema) -> None:
        ...
        assert res.status_code == 201, "status code != 404"
        assert Schema.validate(res.json(), post_schema) is True ,"Not match schema"
